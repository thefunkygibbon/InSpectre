#!/usr/bin/env bash
# =============================================================================
#  InSpectre — Appliance Image Builder  v2.2 (Interactive Edition)
#
#  Produces:
...
check_deps() {
  ...
  $BUILD_VM && deps+=(qemu-img cloud-localds qemu-system-x86_64 qemu-nbd blkid)
...
USERDATA
  ...
  # Expand the partition to prevent the "No space left on device" error
  local root_part=""
  local part_num=""
...
main() {
  echo -e "\n${CYAN}${BOLD}╔══════════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}${BOLD}║    InSpectre Appliance Image Builder  v2.2       ║${NC}"
  echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════════════╝${NC}\n"
