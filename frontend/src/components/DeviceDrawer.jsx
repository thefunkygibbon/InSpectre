...
export function DeviceDrawer({ device, onClose, onRename }) {
  ...
  return (
    <>
      ...
      />
      {/* Drawer */}
      <aside className="fixed right-0 top-0 h-full w-full max-w-md bg-gray-900 border-l border-white/10 z-50 flex flex-col shadow-2xl animate-slide-in overflow-hidden">
              ...
              <div className="space-y-0">
                {scan.open_ports.map((p, i) => (
                  <div key={i} className="flex items-center gap-3 py-1.5 border-b border-white/5 last:border-0">
                    <span className="font-mono text-xs text-brand-400 w-16 shrink-0">{p.port}/{p.proto}</span>
...
function Row({ label, value, mono }) {
  return (
    <div className="flex items-center justify-between py-1.5 gap-4 border-b border-white/5 last:border-0">
